...
def main():
    ...
    for day in structured_days:
        ...
        if start < 0:
            # finalize_navigation_ui may not have run yet; support raw build cards too.
        ...
        # Future structured swim sessions already have their pass structure rendered by
        # finalize_dashboard_ui. The history block is specifically required once a pass
        # has become an outcome so the prescription is never lost.
        if not completed:
            continue
    ...
    print(
        "Workout history OK: genomförda strukturerade simpass behåller sitt passupplägg."
    )
