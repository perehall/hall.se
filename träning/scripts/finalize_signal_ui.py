...
def annotate_week_days(page, plan, activities, today):
    ...
    for day in plan.get("days") or []:
        ...
        fulfilled = day_fulfilled(day, activities or [])
        if day_date < today:
            desired_classes = ["past-completed"] if fulfilled else ["past-exception"]
        elif day_date == today and fulfilled:
            desired_classes = ["past-completed", "today-completed"]
        elif day_date <= horizon_end:
            desired_classes = ["decision-horizon"]
        else:
            desired_classes = ["future-compact"]
        ...
        existing = [
            token
            for token in (match.group("extra") or "").split()
            if token not in ui_state_classes
        ]
        classes = existing + desired_classes
        class_suffix = (" " + " ".join(classes)) if classes else ""
        replacement = f'<div class="day{class_suffix}" id="dag-{day_text}">'
        ...
        if fulfilled and day_date <= today:
            ...
            if count != 1:
                raise RuntimeError(f"Signal-UI: statusbadge saknas för genomförd dag {day_text}")
            page = page[:card_start] + block + page[card_end:]
    return page
