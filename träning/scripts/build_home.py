...
def _km(value):
    if not isinstance(value, (int, float)):
        return ""
    return (f"{value / 1000:.2f}".replace(".", ",") + " km")


def render_development_goals(goal):
    items = [
        item for item in (goal.get("development_goals") or [])
    ...
    for item in items:
        ...
        scope = " · ".join(
            disciplines.get(value, value)
            for value in (item.get("disciplines") or [])
        )
        rows.append(
            '<div class="development-goal-row">'
            f'<div class="performance-goal-kicker">VARAKTIGT MÅL</div>'
            f'<strong>{esc(item.get("label") or item.get("id") or "Utvecklingsmål")}</strong>'
            f'<p>{esc(item.get("objective") or "")}</p>'
            f'<div class="performance-goal-meta">{esc(scope)}</div>'
            '</div>'
        )
    return (
        '<section class="card development-goals" data-development-goals="true">'
        '<div class="title">Varaktigt utvecklingsmål</div>'
        + "".join(rows)
        + '</section>'
    )


def render_performance_goals(goal, today=None):
    items = [
    ...
    for item in items:
        ...
        if event_date:
            ...
            date_label = f"{parsed.day} {MONTH_SHORT[parsed.month]} {parsed.year}"
        heading = " · ".join(part for part in (event, date_label) if part)

        profile = item.get("race_profile") or {}
        facts = [
            _km(profile.get("total_distance_m")),
            f"löp {_km(profile.get('run_distance_m'))}" if _km(profile.get("run_distance_m")) else "",
            f"sim {_km(profile.get('swim_distance_m'))}" if _km(profile.get("swim_distance_m")) else "",
            f"{int(profile['elevation_gain_m'])} hm" if isinstance(profile.get("elevation_gain_m"), (int, float)) else "",
        ]
        fact_line = " · ".join(value for value in facts if value)
        category = item.get("target_category")
        category_line = (
            f"Kategori: {category}"
            if category
            else "Solo/team: ej fastställt"
        )
        countdown = ""
        if today is not None and event_date:
            context = build_competition_context(goal, today)
            if context.get("available") and context.get("goal_id") == item.get("id"):
                countdown = f"{context['days_to_event']} dagar kvar"

        rows.append(
            '<div class="performance-goal-row">'
            f'<div class="performance-goal-kicker">{esc(("A-MÅL · " + sport.upper()) if item.get("priority_class") == "A" and sport else (sport.upper() if sport else "PRESTATIONSMÅL"))}</div>'
            f'<strong>{esc(heading)}</strong>'
            f'<span>{esc(target)}</span>'
            f'<div class="performance-goal-facts">{esc(fact_line)}</div>'
            f'<div class="performance-goal-meta">{esc(" · ".join(x for x in (countdown, category_line) if x))}</div>'
            '</div>'
        )

    return (
        '<section class="card performance-goals" data-performance-goals="true">'
        '<div class="title">Tidsatta prestationsmål</div>'
        + "".join(rows)
        + '</section>'
    )
...
def render_mesocycle(strategy, today):
    ...
    goal_rows = []
    labels_by_goal = {
        item.get("id"): item.get("label") or item.get("event") or item.get("id")
        for item in (strategy.get("goal_contract") or {}).get("goal_set") or []
    }
    for item in mesocycle.get("goal_contributions") or []:
        label = labels_by_goal.get(item.get("goal_id"), item.get("goal_id"))
        goal_rows.append(
            '<div class="goal-contribution-row">'
            f'<strong>{esc(label)}</strong><span>{esc(item.get("contribution") or "")}</span>'
            '</div>'
        )

    details = ""
    if hypothesis or goal_rows:
        details = (
            '<details class="mesocycle-why"><summary>Varför denna mesocykel?</summary>'
            + (f'<p>{esc(hypothesis)}</p>' if hypothesis else "")
            + ('<div class="goal-contributions">' + "".join(goal_rows) + '</div>' if goal_rows else "")
            + '</details>'
        )
...
def build_goal_page(goal, strategy, today):
    ...
    development_goals = render_development_goals(goal)
    performance_goals = render_performance_goals(goal, today)
...
<footer>Målbilden är kanonisk riktning, inte ett prestationsbetyg. · <a href="/cdn-cgi/access/logout" style="color:inherit">Logga ut</a></footer></div></body></html>'''
    ...
    required = [
        ...
        'data-development-goals="true"',
        "Allroundatleten",
        'data-performance-goals="true"',
        "ÖTILLÖ Åland",
        "Topp-10",
        "14 aug 2027",
        "46,54 km",
