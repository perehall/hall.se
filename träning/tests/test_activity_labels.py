...
class ActivityLabelTests(unittest.TestCase):
    ...
    def test_semantic_display_label_wins_over_provider_type(self):
        activity = {
            "sport_type": "MountainBikeRide",
            "display_label": "Enduro",
        }
        self.assertEqual(public_activity_label(activity), "Enduro")

    def test_weight_training_is_never_a_public_label(self):
        activity = {
            "sport_type": "WeightTraining",
            "elapsed_time_s": 1659,
        }
        label = public_activity_label(activity)
        rendered = render_activity_line(activity, label)
